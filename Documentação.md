...
O sistema de mapas é composto por quatro arquivos com responsabilidades distintas:

| Arquivo | Responsabilidade |
|---|---|
| `mapa.hpp` | Structs, flags e API pública em C++ |
| `mapa.cpp` | Renderização, carregamento JSON/Lua, colisão e triggers |
| `mapa.lua` | Módulo Lua para descrever a estrutura do mapa |
| `main.lua` | Importa `mapa.lua`, define sprites, colisores e lógica de triggers |

O Lua define o conteúdo e o comportamento do mapa. O C++ gerencia renderização e cálculos de colisão.

### Flags de Bloco

| Flag | String no Lua | Comportamento |
|---|---|---|
| `MAPA_FLAG_COLISOR` | `"colisor"` | Bloqueia movimento (AABB sólido) |
| `MAPA_FLAG_TRIGGER` | `"trigger"` | Dispara evento de proximidade |
| `MAPA_FLAG_AGUA` | `"agua"` | Tratado como superfície aquática |
| `MAPA_FLAG_ESCADA` | `"escada"` | Permite escalada |
| `MAPA_FLAG_SOMBRA` | `"sombra"` | Bloqueia o FOV |
| `MAPA_FLAG_ANIMADO` | `"animado"` | Tile com animação de frames |

### Tipos de Trigger

| Tipo | String no Lua | Comportamento |
|---|---|---|
| `TRIGGER_TIPO_BAU` | `"bau"` | Coleta ao pressionar `[E]` |
| `TRIGGER_TIPO_NPC` | `"npc"` | Diálogo ao pressionar `[E]` |
| `TRIGGER_TIPO_PORTA` | `"porta"` | Abre/fecha mediante condição |
| `TRIGGER_TIPO_TELEPORTE` | `"teleporte"` | Troca de mapa ao pisar |
| `TRIGGER_TIPO_SCRIPT` | `"script"` | Executa lógica definida em Lua |
| `TRIGGER_TIPO_GENERICO` | `"generico"` | Sem comportamento predefinido |
...
-- Camada de chão (z_order 0 = desenhado primeiro)
...
paredes:tile(2, 2, 1, 2, {"colisor"})
paredes:linha_h(5, 11, 8, 0, 1, {"colisor", "sombra"})
...
m:objeto(1, "bau", 5, 3, 1.5,
    { item="espada_de_fogo", mensagem="Espada de Fogo encontrada." },
    "assets/bau.png")
...
```json
{
  ...
  "objetos": [
    { "id": 1, "tipo": "bau", "col": 5, "lin": 3, "raio": 1.5,
      ...
      "props": { "item": "espada", "mensagem": "Item coletado." } }
  ]
}
```

### Carregamento via FFI
...
while jogo:rodando() do
    ...
    -- Scroll centralizado no jogador
    ...
    if v.resultado == 1 and v.objeto ~= nil then
        local obj = v.objeto
        if obj.tipo == 1 and jogo:tecla_press("e") then
            lib_mapa.mapa_desativar_objeto(mapa, obj.id)
        elseif obj.tipo == 3 and jogo:tecla_press("e") then
            mostrar_dialogo(obj)
        elseif obj.tipo == 4 then
            trocar_mapa(obj)
        end
    end

    jogo:limpar()
    lib_mapa.mapa_desenhar(mapa, jogo._e)
    jogo:desenhar()
    jogo:apresentar()
    jogo:fps(60)
    jogo:atualizar()
...
### Referência da API de Mapa

**Ciclo de vida**

| Função | Parâmetros | Descrição |
|---|---|---|
| `mapa_init(m)` | MapaDados | Inicializa estrutura zerada |
| `mapa_destruir(m)` | MapaDados | Libera recursos |
| `mapa_carregar_lua(m, e, caminho)` | Mapa, Engine, arquivo | Carrega de arquivo Lua |
| `mapa_carregar_json(m, e, caminho)` | Mapa, Engine, arquivo | Carrega de arquivo JSON |

**Renderização**

| Função | Parâmetros | Descrição |
|---|---|---|
| `mapa_desenhar(m, e)` | Mapa, Engine | Renderiza todas as camadas |
| `mapa_desenhar_camada(m, e, idx)` | Mapa, Engine, índice | Renderiza uma camada |
| `mapa_desenhar_objetos(m, e)` | Mapa, Engine | Renderiza objetos com sprite |

**Consulta de tiles**

| Função | Parâmetros | Retorno |
|---|---|---|
| `mapa_colisor(m, col, lin)` | Mapa, coluna, linha | `1` se há colisor |
| `mapa_flags(m, col, lin)` | Mapa, coluna, linha | OR de todos os flags |
| `mapa_get_tile(m, camada, col, lin)` | Mapa, camada, posição | Ponteiro para `MapaTile` |

**Coordenadas**

| Função | Parâmetros | Descrição |
|---|---|---|
| `mapa_pixel_para_tile(m, px, py, col, lin)` | Posição em pixels, saída | Converte pixel → tile |
| `mapa_tile_para_pixel(m, col, lin, x, y)` | Tile, saída | Converte tile → pixel |
| `mapa_set_offset(m, ox, oy)` | Mapa, deslocamento | Define scroll do mapa |
| `mapa_centralizar_em(m, e, col, lin)` | Mapa, Engine, tile | Centraliza câmera |

**Triggers e objetos**

| Função | Parâmetros | Retorno |
|---|---|---|
| `mapa_verificar_trigger(m, px, py)` | Mapa, posição | `MapaVerificacao` |
| `mapa_get_objeto(m, id)` | Mapa, id | Ponteiro para `MapaObjeto` ou `NULL` |
| `mapa_desativar_objeto(m, id)` | Mapa, id | — |

**Modificação em runtime**

| Função | Parâmetros | Descrição |
|---|---|---|
| `mapa_set_tile(m, camada, col, lin, sid, tc, tl, flags)` | Mapa, camada, posição, sprite, tile, flags | Substitui tile |

**API Lua (mapa.lua)**

| Função | Parâmetros | Descrição |
|---|---|---|
| `Mapa.novo(larg, alt, tw, th)` | Dimensões em tiles e pixels | Cria mapa vazio |
| `m:atlas(caminho)` | Arquivo PNG | Define spritesheet principal |
| `m:camada(nome, z_order)` | String, número | Cria camada |
| `m:objeto(id, tipo, col, lin, raio, props, sprite)` | Ver exemplo | Registra trigger |
| `m:build()` | — | Retorna tabela MapData para o C++ |
| `c:tile(col, lin, sc, sl, flags, sprite)` | Posição, tile, flags | Define tile |
| `c:tile_animado(col, lin, frames, fps, flags)` | Posição, frames, fps | Tile animado |
| `c:fill(ci, li, cf, lf, sc, sl, flags)` | Retângulo, tile, flags | Preenche área |
| `c:borda(larg, alt, sc, sl)` | Dimensões, tile | Cria borda sólida |
| `c:linha_h(ci, cf, lin, sc, sl, flags)` | Intervalo horizontal | Linha de tiles |
| `c:linha_v(col, li, lf, sc, sl, flags)` | Intervalo vertical | Coluna de tiles |
