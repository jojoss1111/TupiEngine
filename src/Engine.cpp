...
/* =============================================================================
 * §4  BACKEND  OpenGL 2.1 + X11  (somente quando ENGINE_BACKEND_GL)
 * ============================================================================= */
#ifdef ENGINE_BACKEND_GL

/* ---- Procs VBO (OpenGL 1.5, carregadas via glXGetProcAddressARB) ---------- */
...
/* Tabela de teclas especiais — em ordem de frequência esperada de consulta.
   Não precisa estar ordenada; o compilador armazena em .rodata. */
...
/**
 * Converte um nome de tecla em string para o KeySym correspondente.
 * Complexidade: O(1) para letras/dígitos, O(k_special_keys_count) para especiais.
 */
static KeySym _name_to_keysym(const char *key)
{
    if (!key || !key[0]) return 0;

    /* Tecla de um único caractere alfanumérico → mapeamento direto */
    if (!key[1]) return static_cast<KeySym>(static_cast<unsigned char>(key[0]));

    /* Teclas especiais — busca linear em tabela estática pequena */
...
public:
    ...
    /* Buffers de tecla com swap O(1) — ponteiros trocados a cada frame */
                      ...
                      const char *title, int scale)
{
    ...
    (void)scale; /* escala é tratada via projeção ortográfica */
    ...
    /* Textura branca 1×1 para primitivas sem sprite */
...
void RendererGL::poll_events(Engine *e)
{
    ...
    /* Prepara estado do mouse para o frame */
                                       ...
                                       float r, float g, float b, float a)
{
    ...
    *p++ = dx + dw; *p++ = dy;      *p++ = u1; *p++ = v0; *p++ = r; *p++ = g; *p++ = b; *p++ = a;
    /* Vértice 2: baixo-direito */
    *p++ = dx + dw; *p++ = dy + dh; *p++ = u1; *p++ = v1; *p++ = r; *p++ = g; *p++ = b; *p++ = a;
    /* Vértice 3: baixo-esquerdo */
                                     ...
                                     float rotation_deg, int flip_h, int flip_v)
{
    ...
    glPushMatrix();
    glTranslatef(dx + dw * 0.5f, dy + dh * 0.5f, 0.0f);   /* pivô = centro */
...
#endif
    /* Backends futuros:
    case ENGINE_BACKEND_ID_DX11: return new (std::nothrow) RendererDX11();
    case ENGINE_BACKEND_ID_VK:   return new (std::nothrow) RendererVK();
    */
    default:
        fprintf(stderr, "Engine: backend_id=%d não suportado.\n", backend_id);
        return nullptr;
    }
}

/* =============================================================================
 * §6  MACRO E HELPERS GLOBAIS
 * ============================================================================= */

/** Acessa o IRenderer concreto a partir do ponteiro opaco da Engine. */
#define RENDERER(e)  (static_cast<IRenderer *>((e)->renderer_impl))

/**
 * Calcula as coordenadas UV (normalizadas 0-1) de uma região retangular
 * dentro de uma sprite.  tex_w e tex_h são passados como float para
 * evitar conversões repetidas no caller.
 */
...
/* Sobrecarga de conveniência que aceita SpriteData diretamente */
...
/** Retorna a hitbox efetiva de um objeto (usa hitbox customizada ou bbox do sprite). */
...
/** Encontra o próximo slot livre usando o ring buffer e inicializa a partícula. */
static void _spawn_particle(Engine *e, const ParticleEmitter *em)
{
    /* Ring buffer: parte de particle_next e procura o primeiro slot inativo */
    int idx = -1;
    for (int i = 0; i < ENGINE_MAX_PARTICLES; ++i) {
        ...
        if (!e->particles[candidate].active) {
            ...
            /* Mantém particle_count como high-water mark */
    ...
    /* Geração de valores aleatórios: rand() / RAND_MAX → [0, 1] */
...
void engine_update(Engine *e, float dt)
{
    ...
    if (dt > 0.1f)  dt = 0.1f;   /* cap em 100 ms para evitar explosão de física */
                      ...
                      int box_sid, int tile_w, int tile_h)
{
    ...
    /* Lambda local para empurrar um quad de tile de borda/canto */
...
/* Função auxiliar extra exposta internamente (não declarada no .hpp) */
...
void engine_cap_fps(Engine *, int fps_target)
{
    ...
    /* Primeira chamada: inicializa o timer e retorna */
