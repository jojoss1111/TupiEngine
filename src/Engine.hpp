 ...
 *
 * O arquivo é compilável tanto como C puro (sem IRenderer/QuadParams)
 * quanto como C++, graças aos guards #ifdef __cplusplus.  Isso mantém
 * compatibilidade total com o FFI LuaJIT, que enxerga apenas o lado C.
 *
 ...
 *
 * Cada backend (GL, DX11, VK) implementa esta classe.  A Engine acessa o
 * renderer concreto via renderer_impl com um cast estático; o Lua FFI
 * nunca interage com esta interface diretamente.
 *
 * Convenções:
 *   • flush()      → envia o batch pendente de quads para a GPU.
 *   • present()    → swap de buffers / apresentação da swapchain.
 *   • push_quad()  → acumula no batch; set_texture() pode disparar um flush
 *                    interno se a textura mudou.
...
struct QuadParams {
    float dx, dy, dw, dh;  /* posição e tamanho em pixels virtuais                   */
    float u0, v0, u1, v1;  /* UVs normalizados 0..1 (topo-esquerdo → baixo-direito)  */
    float r, g, b, a;      /* cor/tint e alpha                                        */
...
public:
    ...
    /* Controle de frame */
    virtual void clear(Engine *e) = 0;   /* limpa o framebuffer com a cor de fundo */
    virtual void flush(Engine *e) = 0;   /* envia o batch pendente                 */
    virtual void present(Engine *e) = 0; /* apresenta o frame na tela              */
    virtual void set_vsync(bool enable) = 0;

    /* Upload/remoção de texturas na GPU */
    virtual unsigned int upload_texture(const unsigned char *rgba,
                                        unsigned int w,
                                        unsigned int h) = 0;
    virtual void delete_texture(unsigned int handle) = 0;

    /* Batch de quads com textura */
    virtual void set_texture(Engine *e, unsigned int tex) = 0;
    ...
    /* Transformação de câmera para objetos no espaço do mundo */
    virtual void camera_push(Engine *e) = 0;
    virtual void camera_pop(Engine *e) = 0;

    /*
     * Primitivas que precisam de acesso direto à pipeline de rasterização.
    ...
    /* Modos de blend — inverted para efeito de negativo/XOR visual */
    virtual void set_blend_inverted(Engine *e) = 0;
    virtual void set_blend_normal(Engine *e) = 0;

    /* Define a cor de limpeza; aplicada na próxima chamada a clear() */
 ...
 *
 * Instancia o IRenderer concreto correspondente a backend_id.
 * Retorna nullptr se o backend não foi compilado neste binário.
 * Implementada em Engine.cpp.
 */
