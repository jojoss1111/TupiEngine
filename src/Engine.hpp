...
/* =============================================================================
 * Layers de Colisão — máscaras de bits para engine_check_collision*
 *
 * collision_layer : camadas a que este objeto pertence  (quem sou eu?)
 * collision_mask  : camadas com as quais este objeto interage (com quem colido?)
 *
 * Dois objetos A e B colidem apenas se:
 *   (A.collision_layer & B.collision_mask) != 0
 *   (B.collision_layer & A.collision_mask) != 0
 *
 * Predefinições prontas para uso; o jogo pode criar suas próprias a partir
 * de (1 << N) com N entre 0 e 30.
 *
 *   LAYER_DEFAULT  — objetos sem categoria específica
 *   LAYER_PLAYER   — personagem controlado pelo jogador
 *   LAYER_ENEMY    — inimigos / NPCs hostis
 *   LAYER_BULLET   — projéteis do jogador
 *   LAYER_WALL     — geometria estática (paredes, chão, plataformas)
 *   LAYER_ITEM     — colecionáveis, interativos
 *   LAYER_SENSOR   — triggers / zonas de evento (sempre sensor = 1)
 *   LAYER_ALL      — máscara que aceita colisão com qualquer layer
 *
 * Exemplo — bala do jogador: não colide com outras balas, só com inimigos e paredes:
 *   obj.collision_layer = ENGINE_LAYER_BULLET;
 *   obj.collision_mask  = ENGINE_LAYER_ENEMY | ENGINE_LAYER_WALL;
 * ============================================================================= */
#define ENGINE_LAYER_DEFAULT   (1u << 0)
#define ENGINE_LAYER_PLAYER    (1u << 1)
#define ENGINE_LAYER_ENEMY     (1u << 2)
#define ENGINE_LAYER_BULLET    (1u << 3)
#define ENGINE_LAYER_WALL      (1u << 4)
#define ENGINE_LAYER_ITEM      (1u << 5)
#define ENGINE_LAYER_SENSOR    (1u << 6)
#define ENGINE_LAYER_ALL       (0xFFFFFFFFu)

/* =============================================================================
 * Flag de sensor
 *
 * Quando is_sensor == 1, a colisão dispara o evento (engine_sgrid_*_collision
 * retorna o par) mas NÃO bloqueia o movimento.  A lógica de separação fica
 * inteiramente no jogo — a engine apenas reporta o overlap.
 *
 * Sensores são úteis para:
 *   • Zonas de dano / trigger de cutscene
 *   • Itens colecionáveis no chão
 *   • Checkpoints, portais, áreas de spawn
 * ============================================================================= */
/* (o campo is_sensor vive em GameObject — veja abaixo) */
 ...
 * nos backends DX11.  O FFI Lua trata como unsigned int e nunca o
...
typedef struct {
    ...
    unsigned int  collision_layer;  /* máscara: a qual(ais) layer(s) este obj pertence  */
    unsigned int  collision_mask;   /* máscara: com quais layers ele interage           */

    /* --- Tipo de colisão -------------------------------------------------- */
    int           is_sensor;  /* 0 = sólido (bloqueia movimento)                       */
                              /* 1 = sensor (apenas dispara evento, não bloqueia)      */
} GameObject;
 ...
 *
 * O grid usa std::unordered_map com chave (col, row) para suportar:
 *   • Coordenadas negativas (jogador viaja para x < 0 ou y < 0).
 *   • Mapas virtualmente ilimitados — células são alocadas sob demanda.
 *   • Sem desperdício de memória para regiões vazias.
 *
 * A chave é um int64_t que encoda (col, row):
 *   key = ((int64_t)(int32_t)col << 32) | (uint32_t)(int32_t)row
 *
 * ENGINE_SGRID_CELL_SIZE e ENGINE_SGRID_BUCKET_CAP são mantidos pois ainda
 * controlam a granularidade e a capacidade por célula.
 *
 * As constantes COLS/ROWS e TOTAL_CELLS eram exclusivas do grid estático;
 * são preservadas apenas para compatibilidade binária / FFI legacy.
 * O código de runtime NÃO as usa mais — a capacidade é dinâmica.
 * ============================================================================= */
#define ENGINE_SGRID_CELL_SIZE   64   /* largura/altura de cada célula em px          */
#define ENGINE_SGRID_COLS        64   /* mantido por compatibilidade (não usado)       */
#define ENGINE_SGRID_ROWS        64   /* mantido por compatibilidade (não usado)       */
 ...
 *
 * cell_keys[] armazena as chaves int64_t das células onde o objeto está
 * registrado (uma por célula, máximo ENGINE_SGRID_OBJ_MAX_CELLS).
 */
typedef struct {
    int64_t cell_keys[ENGINE_SGRID_OBJ_MAX_CELLS]; /* chaves das células ocupadas */
    int     count;
} SpatialObjEntry;

/*
 * SpatialGrid — estado completo do grid espacial dinâmico.
 *
 * O mapa de células fica no heap (std::unordered_map), portanto a struct
 * em si é pequena.  O ponteiro cells_map é opaco para o Lua/FFI — nunca
 * acesse diretamente; use sempre as funções engine_sgrid_*().
 *
 * Compatibilidade FFI: o campo cells_map é void* (tamanho de ponteiro),
 * mantendo a struct C-compatible mesmo que o map interno seja C++.
 */
typedef struct {
    void           *cells_map;  /* std::unordered_map<int64_t, SpatialCell>* (heap) */
    ...
    /* cols/rows removidos: o grid é agora ilimitado em todas as direções   */
} SpatialGrid;
...
typedef struct {
    ...
    int         backend_id;    /* ENGINE_BACKEND_ID_GL / DX11 /                      */
...
int  engine_is_sensor(Engine *e, int oid);    /* 1 se o objeto é sensor */
