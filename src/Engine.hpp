...
/* ============================================================
 * Structs de dados — POD, C-compatible (FFI seguro)
 * ============================================================ */
...
typedef struct {
    ...
    GLuint texture;
#else
    unsigned int texture;   /* handle opaco para outros backends */
#endif
    int    width;
    int    height;
    int    loaded;
...
/* ============================================================
 * Engine — contexto principal (C-compatible para FFI)
 *
 * renderer_impl  → ponteiro opaco para o IRenderer concreto.
 *                  O FFI Lua enxerga void*, não precisa saber o tipo.
 * backend_id     → ENGINE_BACKEND_ID_GL / DX11 / VK
 * ============================================================ */
typedef struct {
    /* ---- Ponteiro opaco para o backend concreto ----------- */
    void       *renderer_impl;  /* IRenderer* — alocado em engine_init */
    int         backend_id;
    ...
    /* ---- Textura branca (primitivas sem sprite) ------------ */
...
/* ============================================================
 * IRenderer — interface C++ de backend gráfico
 *
 * Cada backend implementa esta classe abstrata. A Engine
 * chama os métodos via ponteiro renderer_impl (cast interno
 * em Engine.cpp). O FFI Lua nunca toca nesta interface.
 * ============================================================ */
#ifdef __cplusplus
...
struct QuadParams {
    ...
    float u0, v0, u1, v1;
    float r, g, b, a;
    float rotation;    /* graus; 0 = sem rotação */
    int   flip_h, flip_v;
};
...
public:
    ...
    /* Frame */
    virtual void clear(Engine *e) = 0;
    virtual void flush(Engine *e) = 0;
    virtual void present(Engine *e) = 0;
    ...
    /* Primitivas que precisam de acesso direto à pipeline */
    ...
    /* Blend mode para efeito invertido */
    virtual void set_blend_inverted(Engine *e) = 0;
    virtual void set_blend_normal(Engine *e) = 0;

    /* Polling de eventos — específico de plataforma */
    virtual void poll_events(Engine *e) = 0;
};

/* ---- Factory — cria o renderer correto para o backend ------ */
IRenderer *engine_create_renderer(int backend_id);

#endif /* __cplusplus */

/* ============================================================
 * API pública — extern "C" para compatibilidade total com C / FFI
 * Nomes de funções idênticos à versão anterior.
 * ============================================================ */
#ifdef __cplusplus
extern "C" {
#endif
