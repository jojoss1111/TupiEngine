 ...
 * collision_layer / collision_mask: bitmasks de camada de colisão.
 * is_sensor: 1 = sensor (apenas reporta overlap, não bloqueia movimento). */
typedef struct {
    int           x, y;
    int           sprite_id;
    unsigned long color;
...
/* Spatial Grid dinâmico
 *
 * cells_map é void* — ponteiro opaco para o std::unordered_map interno.
 * NUNCA acesse diretamente do Lua; use sempre engine_sgrid_*().
 *
 * obj_cells rastreia em quais células cada objeto está inscrito;
 * cell_keys[] guarda as chaves int64_t (col,row codificadas).
 */
static const int ENGINE_SGRID_CELL_SIZE  = 64;
static const int ENGINE_SGRID_COLS       = 64;   /* mantido por compatibilidade */
static const int ENGINE_SGRID_ROWS       = 64;   /* mantido por compatibilidade */
static const int ENGINE_SGRID_BUCKET_CAP = 16;
static const int ENGINE_SGRID_TOTAL_CELLS = 4096;
 ...
 * cell_keys[] usa int64_t para suportar coordenadas negativas. */
typedef struct {
    int64_t cell_keys[4];   /* ENGINE_SGRID_OBJ_MAX_CELLS */
    int     count;
} SpatialObjEntry;

/* Grid dinâmico — cells_map é ponteiro opaco */
typedef struct {
    void           *cells_map;      /* std::unordered_map* opaco — não acesse */
...
/* Constantes de camada de colisão */
static const unsigned int ENGINE_LAYER_DEFAULT = 1;
static const unsigned int ENGINE_LAYER_PLAYER  = 2;
static const unsigned int ENGINE_LAYER_ENEMY   = 4;
static const unsigned int ENGINE_LAYER_BULLET  = 8;
static const unsigned int ENGINE_LAYER_WALL    = 16;
static const unsigned int ENGINE_LAYER_ITEM    = 32;
static const unsigned int ENGINE_LAYER_SENSOR  = 64;
static const unsigned int ENGINE_LAYER_ALL     = 4294967295; /* 0xFFFFFFFF */
